...
Self_Clean::Self_Clean(){
	this->clean_map=NULL;
	diffx=0,diffy=0,rotatedegree=0;
}
...
bool Self_Clean::is_arrive(Pointf robot_pose){
	//Point *p=clean_map_p->previous->point;
	//ROS_INFO("is_arrive");
	return /*(robot_pose.x==aim.x && robot_pose.y==aim.y);*/(robot_pose.x>=(double)(aim.x-1.5) && robot_pose.x<=(double)(aim.x+1.5) && robot_pose.y>=(double)(aim.y-1.5) && robot_pose.y<=(double)(aim.y+1.5));
}
...
bool Self_Clean::send_point(ros::Publisher point_pub){
	...
	if(this->clean_map_p!=NULL){
		...
		Pointf t=tfmtos(Pointf((double)x*this->map_resolution,(double)y*this->map_resolution);
		goal.header.frame_id="map";
		goal.header.stamp=ros::Time::now();
		goal.pose.position.x=t.x;
		goal.pose.position.y=t.y;
		goal.pose.orientation.w = 1.0;
		ROS_INFO("go to %d,%d,%f,%f",x,y,t.x,t.y);
...
Pointf Self_Clean::tfmtos(Pointf m){
	Pointf newp=Pointf();
    /*newp.x=(m.x-diffx)*cos(this->rotatedegree)+(m.y-diffy)*sin(this->rotatedegree);
    newp.y=-(m.x-diffx)*sin(this->rotatedegree)+(m.y-diffy)*cos(this->rotatedegree);*/
	newp.x=(m.x-diffx);
    newp.y=(m.y-diffy);
    return newp;
}