...
bool PathPlan::cb_service(nav_msgs::GetPlanRequest &req, nav_msgs::GetPlanResponse &res)
{
  ...
  if (req.start.header.frame_id != this->map->header.frame_id)
  {
    ROS_WARN("Destination and map have different frame");
    std::cout << "Point frame: " << req.start.header.frame_id << endl;
    std::cout << "Map frame: " << this->map->header.frame_id << endl;
    return false;
  }
